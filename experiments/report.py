...
def load(pattern: str) -> list[dict]:
    ...
    for path in sorted(glob.glob(os.path.join(RESULTS, pattern))):
        if path.endswith("fragmentation.json"):
            continue
...
def fragmentation_section() -> str:
    ...
    with open(path) as fh:
        data = json.load(fh)
    rows = [[r["soup_size"], r["capacity"], r["alive"], r["births"],
             r["alloc_failures"], r["alloc_failures_per_birth"],
             r["genotypes_seen"], r["mean_size"]] for r in data["rows"]]
    return table(["soup cells", "capacity", "alive", "births", "mal failures",
                  "per birth", "genotypes seen", "mean size"], rows)
...
def main() -> None:
    ...
    long_runs = [r for r in runs if r["config"]["instructions"] > 100_000_000]
